...
class MapController extends GetxController {
  ...
  void loadOrderData() {
    try {
      final data = FirebaseFirestore.instance
          ...
          .where(OrderModel.orderIdString,
              isEqualTo: newOrderController.newOrderId.value)
          .snapshots();
      data.listen((result) {
        if (result.docs.isNotEmpty) {
          final orders =
              result.docs.map((e) => OrderModel.fromMap(e.data())).toList();
          _orderData.value = RemoteData<List<OrderModel>>(
              status: RemoteDataStatus.success, data: orders);
  ...
  void loadMerchantData() {
    try {
      final data = FirebaseFirestore.instance
          .collection(MerchantModel.collectionName)
          .where(MerchantModel.merchantIdString, isEqualTo: merchantId.value)
          .snapshots();
      data.listen((result) {
        final merchant =
            result.docs.map((e) => MerchantModel.fromMap(e.data())).toList();
        _merchantData.value = RemoteData<List<MerchantModel>>(
            status: RemoteDataStatus.success, data: merchant);

        merchantPosition = LatLng(
            merchant[0].position.latitude, merchant[0].position.longitude);
        myPosLatLng.add(merchantPosition);
        initMerchantMarker()
            .then((marker) => markers = marker as RxList<Marker>);
  ...
  void loadCustomerData() {
    try {
      final data = FirebaseFirestore.instance
          ...
          .where(CustomerModel.customerIdString, isEqualTo: customerId.value)
          .snapshots();
      data.listen((result) {
        final customer =
            result.docs.map((e) => CustomerModel.fromMap(e.data())).toList();
        _customerData.value = RemoteData<List<CustomerModel>>(
            status: RemoteDataStatus.success, data: customer);

        customerPosition = LatLng(
            customer[0].position.latitude, customer[0].position.longitude);
        myPosLatLng.clear();
